...
FastAPI + SQLite. 엔드포인트 24개 · 테이블 30개 + 뷰 1개.
...
> `GET /api/report/settings`, `GET /api/admin/{instruction,sources,uploads,members}`,
...
python -m app.db.seed_users          # 로그인 계정 (초기 비밀번호 1111)
```
...
python -m pytest tests/ -q           # 현재 84개
```
...
| 트렌드 테마 | `theme` | |
| 근거 자료 | `feed_item` | 파밍 파이프라인이 채우는 테이블과 동일. 화면용 컬럼(`kind_label`·`theme`·`biz_hint`·`is_new`) 추가 |
...
| 관리자 | `crawl_source` · `upload_file` · `admin_member` · `app_setting` | |
...
```bash
python -m app.db.seed_users                    # 기본 데모 계정 3개
...
- `OI_TODAY` 는 데모 시드 정합을 위해 `2026-07-30` 로 고정돼 있습니다. 과제 발굴의 최근
  30일 창과 **트렌드룸 기간 필터의 기준일**이 모두 이 값을 씁니다. 실제 크롤 데이터로
  돌리려면 `.env` 에서 **빈 값**으로 두세요(= 오늘). 고정값이면 오늘 수집한 항목이
  창 밖으로 밀려 제외됩니다.
